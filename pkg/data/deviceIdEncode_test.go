...
var testUUIDCode = []struct {
	...
	{"00003e70-3e54-11ec-8586-3daab251eca2", "x7fv87xziKWPAgNrNQZ222"},
	...
	{"00000000-0000-0000-8000-000000000000", "z7C8BNkRBVT22222222222"},
	{"00000000-0000-0001-0000-000000000000", "yDNELiVqLxt22222222222"},
}

func TestErrorDeviceIdEncode(t *testing.T) {
	t.Parallel()

	d := data.NewData()

	decode, err := d.DeviceIDEncode("")

	assert.Equal(t, 0, len(decode))
	assert.Contains(t, err.Error(), "error in deviceID Encode")
}

func TestSuccessErrorDeviceIdEncode(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testUUIDCode {
		...
		t.Run("Test success", func(t *testing.T) {
			...
			shortUUID, _ := d.DeviceIDEncode(tc.uuid)
			assert.Equal(t, 22, len(shortUUID))
			assert.Equal(t, tc.shortUUID, shortUUID)
		})
	}
}
